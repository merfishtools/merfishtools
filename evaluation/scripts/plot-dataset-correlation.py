...
ax.scatter(small, large, cmap=mpl.cm.get_cmap("Reds"), label="conditional expectation", edgecolors="face", norm=mpl.colors.Normalize())#vmin=5, vmax=50))
ax.plot([min_value, max_value], [min_value, max_value], "k--")
ax.scatter(small_counts, large_counts, cmap=mpl.cm.get_cmap("Greys"), label="raw counts", edgecolors="face", norm=mpl.colors.Normalize())#vmin=5, vmax=50))
ax.set_xscale("log")
ax.set_yscale("log")
