...
fn prob_call_mismatch(N: u8, m: u8, p0: Prob, p1: Prob) -> Prob {
    (xi(1, 0, N, m, p0, p1) + xi(0, 1, N, m, p0, p1)) / 2.0
}
...
fn prob_miscall_mismatch(N: u8, m: u8, p0: Prob, p1: Prob) -> Prob {
    (xi(2, 1, N, m, p0, p1) + xi(1, 2, N, m, p0, p1) + xi(2, 3, N, m, p0, p1) + xi(3, 2, N, m, p0, p1)) / 4.0
}
...
mod tests {
    ...
    use super::{prob_call_exact, prob_call_mismatch, prob_miscall_exact, prob_miscall_mismatch, prob_missed};
    ...
    fn test_prob_call_mismatch() {
        ...
        assert!(p.approx_eq(&0.03070824715017128));
    }
    ...
    fn test_prob_miscall_mismatch() {
        ...
        println!("{}", p);
        assert!(p.approx_eq(&0.0000714129975950271));
    }
    ...
    fn test_prob_missed() {
        ...
        println!("{}", p);
        assert!(p.approx_eq(&0.00010447408578189072));
    }
}
